...
module x_end_motor_base(){
 x_end_base();
 translate(v=[x_rod_center,(44+bearing_size)/2,motor_axis_z-4]) cube(size = [17,44,51], center = true);
}

module x_end_motor_holes(){
 x_end_holes();
 // Position to place
 translate(v=[x_rod_center,(44+bearing_size)/2,motor_axis_z]){
  // Belt hole
  translate(v=[0,1,0]) cube(size = [10,46,22], center = true);
...
x_end_motor();
 translate(v=[x_rod_center,(44+bearing_size)/2,motor_axis_z+4]){
